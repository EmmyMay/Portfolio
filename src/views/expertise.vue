<template>
  <div class="about">
    <h1 class="jwn">Services I render:</h1>

    <!-- This is for the MD Viewport -->
    <v-layout class="shiftPanel" justify-center row wrap>
      <v-flex d-flex xs8 sm8 md8>
        <v-expansion-panel dark focusable popout>
          <v-expansion-panel-content
            color="primary"
            class="designPanel shiftPanel2"
            v-for="data in eData"
            :key="data.mainService"
          >
            <template v-slot:header>
              <div class="title bgColor">{{data.mainService}}</div>
            </template>
            <v-card class="designPanel" color="black">
              <v-card-text
                class="subheading vcardtxt"
                v-for="subservice in data.subService"
                :key="subservice"
              >{{subservice}}</v-card-text>
            </v-card>
          </v-expansion-panel-content>
        </v-expansion-panel>
      </v-flex>
    ...
    <router-link to="/">
      <v-btn id="backhome"  color="black">Home</v-btn>
    </router-link>
